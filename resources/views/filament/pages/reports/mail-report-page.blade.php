<x-filament-panels::page>
    <div class="space-y-6 fi-section gap-6">
        ...
        <div class="fi-section overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
            ...
            {{-- Sayfa başına kayıt + Tablo sarmalayıcı (taşma sadece bu blokta) --}}
            @if($banksPaginator)
                <div class="px-4 sm:px-6 py-2 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-2 bg-gray-50/50 dark:bg-gray-800/50">
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        Toplam <strong>{{ $banksPaginator->total() }}</strong> kayıt
                    </p>
                </div>
            @endif
                ...
                <div class="overflow-x-auto overflow-y-visible" style="overflow-x: auto; -webkit-overflow-scrolling: touch;">
                    <table class="fi-table w-full table-auto divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900">
                            @if($banksPaginator && $banksPaginator->count() > 0)
                                @foreach($banksPaginator as $bank)
                                    <tr class="fi-table-row divide-x divide-gray-200 dark:divide-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                        <td class="fi-table-cell px-4 py-3 text-sm text-gray-900 dark:text-white whitespace-nowrap">{{ $bank->customer?->name ?? '-' }}</td>
                                        <td class="fi-table-cell px-4 py-3 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">{{ $bank->bank_name }}</td>
                                        <td class="fi-table-cell px-4 py-3 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">{{ $bank->request?->year ?? '-' }}</td>
                                        ...
                                        <td class="fi-table-cell px-4 py-3 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">{{ $bank->reply_received_at?->format('d.m.Y H:i') ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
            ...
            @if($banksPaginator)
                <div class="fi-section-footer flex flex-wrap items-center justify-between gap-2 border-t border-gray-200 dark:border-gray-700 px-4 py-3 sm:px-6 bg-gray-50/50 dark:bg-gray-800/50">
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        @if($banksPaginator->total() > 0)
                            {{ $banksPaginator->firstItem() }} – {{ $banksPaginator->lastItem() }} / {{ $banksPaginator->total() }}
                        @else
                            0 kayıt
                        @endif
                    </p>
